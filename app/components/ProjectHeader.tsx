...
export default function ProjectHeader({
    ...
    return (
        <header className="space-y-6">
            <a
                href={backHref}
                className="inline-flex text-sm font-semibold text-zinc-900 underline decoration-zinc-300 underline-offset-4 hover:decoration-zinc-900"
            >
                ← Geri dön
            </a>
            ...
            <div className="flex flex-wrap gap-3">
                {demoHref ? (
                    ...
                    <span className="rounded-xl bg-zinc-100 px-5 py-3 text-sm font-semibold text-zinc-500">
                        Demo yakında
                    </span>
                )}
                ...
                {repoHref ? (
                    ...
                    <span className="rounded-xl border border-zinc-200 bg-white px-5 py-3 text-sm font-semibold text-zinc-500">
                        Repo yakında
                    </span>
                )}
            </div>

            {/* Görsel alanı (şimdilik placeholder) */}
            <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-10 text-center text-sm text-zinc-500">
                Proje görseli / ekran görüntüsü alanı
            </div>
        </header>
    );
}
